...
def docx_format_01(file_path):
    ...
    # 首行处理（标题行）
    # 居中 不缩进
    new_docx.paragraphs[0].alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
    new_docx.paragraphs[0].paragraph_format.first_line_indent = 0
    # 字体、字号调整
    for r in new_docx.paragraphs[0].runs:
        r.font.name = u'方正小标宋简体'
        r._element.rPr.rFonts.set(qn('w:eastAsia'), u'方正小标宋简体')
        r.font.bold = False
        r.font.size = Pt(22)
    # 插入一个空行
    new_docx.paragraphs[1].insert_paragraph_before('')
